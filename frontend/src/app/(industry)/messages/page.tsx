...
const CONTACTS = [
  { id: "00000000-0000-0000-0000-000000000001", emoji: "👨‍🌾", name: "Ramesh Patil", location: "Nashik, MH", online: true },
];
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      const { data, error } = await supabase
        ...
        .select("*")
        .or(
          `and(sender_id.eq.${DEMO_INDUSTRY_ID},receiver_id.eq.${activeId}),and(sender_id.eq.${activeId},receiver_id.eq.${DEMO_INDUSTRY_ID})`
        )
    ...
    const channel = supabase
      .channel(`industry-chat-${activeId}`)
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "messages" }, (payload) => {
        const msg = payload.new as any;
        const isRelevant =
          (msg.sender_id === DEMO_INDUSTRY_ID && msg.receiver_id === activeId) ||
          (msg.sender_id === activeId && msg.receiver_id === DEMO_INDUSTRY_ID);
  ...
  return (
    <>
      <div style={{ background: "rgba(255,255,255,0.85)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(30,70,20,0.07)", padding: "16px 32px", position: "sticky", top: 0, zIndex: 40 }}>
        ...
        <p style={{ fontSize: 13, color: "#6b7280", marginTop: 1 }}>Direct chats with verified farmers</p>
      </div>
      ...
      <div style={{ padding: "28px 32px", flex: 1 }} className="fade-up">
        <div style={{ display: "grid", gridTemplateColumns: "280px 1fr", gap: 20, height: "calc(100vh - 180px)" }}>
          ...
          <div style={{ background: "#fff", borderRadius: 20, padding: 16, border: "1px solid rgba(30,70,20,0.07)", overflowY: "auto" }}>
            <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 15, fontWeight: 600, color: "#1a2e1a", marginBottom: 14, padding: "0 4px" }}>Farmers</div>
            {CONTACTS.map(c => (
          ...
          <div style={{ background: "#fff", borderRadius: 20, border: "1px solid rgba(30,70,20,0.07)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
            <div style={{ padding: "16px 20px", borderBottom: "1px solid #f0f0f0", display: "flex", alignItems: "center", gap: 12, background: "#fff" }}>
              ...
              <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
                ...
                <button style={{ padding: "7px 14px", borderRadius: 10, border: "none", background: "#f0f7f0", fontSize: 12, fontWeight: 600, color: "#2d6b30", cursor: "pointer", fontFamily: "'DM Sans',sans-serif" }}>📋 View Bids</button>
              </div>
            </div>
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: "20px", display: "flex", flexDirection: "column", gap: 10, background: "#fafaf8" }}>
              {messages.length === 0 && (
                <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", color: "#9ca3af", gap: 8 }}>
                  ...
                  <div style={{ fontSize: 12 }}>Start the conversation with {active.name}!</div>
                </div>
              )}

              {messages.map((msg, i) => {
                const isMe = msg.sender_id === DEMO_INDUSTRY_ID;
                ...
                return (
                  <div key={i} style={{ display: "flex", justifyContent: isMe ? "flex-end" : "flex-start" }}>
                    ...
                    <div style={{ maxWidth: "65%", padding: "11px 15px", borderRadius: isMe ? "18px 18px 4px 18px" : "18px 18px 18px 4px", background: isMe ? "linear-gradient(135deg,#1e2a4a,#2d3d6b)" : "#fff", color: isMe ? "#fff" : "#1a2e1a", fontSize: 13, lineHeight: 1.55, boxShadow: "0 2px 8px rgba(0,0,0,0.07)" }}>
                      {msg.text}
            ...
            <div style={{ padding: "14px 16px", borderTop: "1px solid #f0f0f0", display: "flex", gap: 10, alignItems: "center", background: "#fff" }}>
              ...
              <button onClick={send} disabled={sending || !input.trim()}
                style={{ padding: "12px 22px", borderRadius: 12, border: "none", background: sending || !input.trim() ? "#9ca3af" : "linear-gradient(135deg,#1e2a4a,#2d3d6b)", color: "#fff", fontWeight: 700, cursor: sending || !input.trim() ? "not-allowed" : "pointer", fontSize: 13, fontFamily: "'DM Sans',sans-serif" }}>
                {sending ? "⏳" : "Send →"}
              </button>
            </div>
          </div>
